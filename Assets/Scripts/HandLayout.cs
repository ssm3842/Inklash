...
{
    [SerializeField] HandCardSlot[] cardSlots;
    ...
    public void AddCardToHand(GameObject cardObject)
    {
        ...
        foreach (HandCardSlot slot in cardSlots)
        {
            if (slot.isEmpty)
            {
                slot.isEmpty = false;
                cardObject.GetComponent<Card>().slot = slot;
                cardRect.position = slot.GetComponent<RectTransform>().position + new Vector3(0, 145, 0);
                break;
            }
        }
    }
